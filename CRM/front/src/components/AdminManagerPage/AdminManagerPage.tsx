...
const AdminManagerPage: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:3001/api/users/managers', {
        headers: {
  ...
  const handleChangeRole = async (userId: number) => {
    try {
      const token = localStorage.getItem('authToken');
      await axios.patch(`http://localhost:3001/api/users/${userId}/role`, { role: 'employee' }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
        ...
        {users.map(user => (
          <div key={user.id} className="user-card">
            ...
            <div className="actions">
              ...
              <FaExchangeAlt className="icon" onClick={() => handleChangeRole(user.id)} />
            </div>
          </div>
        ))}
      </div>
