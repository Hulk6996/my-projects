...
const AdminEmployeePage: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://localhost:3001/api/users/employees', {
        headers: {
  ...
  const handleChangeRole = async (userId: number) => {
    try {
      await axios.patch(`http://localhost:3001/api/users/${userId}/role`, { role: 'manager' }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('authToken')}`,
        },
      });
